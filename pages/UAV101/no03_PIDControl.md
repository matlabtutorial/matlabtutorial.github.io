...
이렇듯, Motor Mixing Matrix는 일종의 함수로써, [추력, 롤, 피치, 요] 커맨드 입력 받아 모터의 커맨드를 출력해주는 역할을 한다.
...
쿼드콥터가 지구 고정 좌표계(Global Frame)에서 원하는 X-Y 위치로 이동하려면, 현재 자신의 방향(요 각도)에 맞춰서 몸체를 기울여야 한다. (몸체의 z 축에 맞게 회전시킨다는 의미가 아니라, pitch 혹은 roll 움직임으로 몸체를 이동시킨다는 뜻이다.) 

이때 $dx_g$ (지구 고정 X축 방향 움직임 요구량)와 $dy_g$ (지구 고정 Y축 방향 움직임 요구량)를 기체 고정 좌표계(Body Frame)에서의 `roll desired`와 `pitch desired`로 변환하는 과정이 필요하다. 이는 **좌표계 회전 변환**을 통해 이루어진다.

만약 지구 고정 좌표계에서의 원하는 움직임 성분($dx_g$, $dy_g$)이 있고, 쿼드콥터의 현재 요(Yaw) 각도($\psi$)가 있다면, 이를 기체 고정 좌표계에서의 움직임 성분($dx_b$, $dy_b$)으로 변환하는 회전 행렬은 다음과 같다:

$$\begin{pmatrix} 
  dx_b \\ dy_b \end{pmatrix} = 
  \begin{pmatrix} \cos(\psi) & \sin(\psi) \\ 
                  -\sin(\psi) & \cos(\psi) 
  \end{pmatrix} 
  \begin{pmatrix} dx_g \\ dy_g \end{pmatrix} % 식 (2)$$

이 식을 풀면 다음과 같다:
* $dx_b = dx_g \times \cos(\psi) + dy_g \times \sin(\psi)$
* $dy_b = -dx_g \times \sin(\psi) + dy_g \times \cos(\psi)$

여기서 $dx_b$는 기체의 전후 방향으로의 움직임을 유발하는 기울임(피치)에 주로 사용되고, $dy_b$는 기체의 좌우 방향으로의 움직임을 유발하는 기울임(롤)에 주로 사용된다.

그런데, 식 (2)를 잘 보면 곱해지는 행렬은 $\psi$ 라는 Yaw 각도로 역회전시키는 행렬이다. yaw 각도 $\psi$ 에 맞게 역회전을 시키는 것은 Yaw는 지구 좌표계의 z축 기준 기체 좌표계의 z축이 얼마나 회전했냐는 것을 말하고 있기 때문에 Yaw가 회전한 만큼 지구 좌표계에 맞게 "돌려놓아야 하기 때문"이다. 예를 들어, 북쪽에 맛집이 있는데, 나는 동쪽을 향해보고 있다고 하면 내 몸은 북쪽 기준 90도 회전해있는 것이다. 이 때, 내 몸은 -90도 방향으로 이동할 것을 생각해야 한다. 좀 더 구체적인 예제로 알아보자.

### 예제 1. 현재 위치 (0, 0), 목표 위치 (1, 0), 현재 Yaw = 0도
...
이제 역회전 행렬을 곱해서 pitch와 roll correction을 위한 값을 계산해보면,

$$  \begin{pmatrix} \cos(\psi) & \sin(\psi) \\ -\sin(\psi) & \cos(\psi)\end{pmatrix} \begin{pmatrix} dx \\ dy \end{pmatrix} = 
 \begin{pmatrix} \cos(\psi) & \sin(\psi) \\ -\sin(\psi) & \cos(\psi)\end{pmatrix} \begin{pmatrix} 1 \\ 0 \end{pmatrix}$$

$$=\begin{pmatrix} \cos(\psi) \\ -\sin(\psi) \end{pmatrix} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$$

즉, pitch를 보정하기 위한 값은 1, roll을 보정하기 위한 값은 0이 된다. 따라서 드론은 앞으로 나아가게 된다.

### 예제 2. 현재 위치 (0, 0), 목표 위치 (1, 0), 현재 Yaw = 90도
...
$$  \begin{pmatrix} \cos(\psi) & \sin(\psi) \\ -\sin(\psi) & \cos(\psi)\end{pmatrix} \begin{pmatrix} dx \\ dy \end{pmatrix} = 
 \begin{pmatrix} \cos(\psi) & \sin(\psi) \\ -\sin(\psi) & \cos(\psi)\end{pmatrix} \begin{pmatrix} 3 \\ 2 \end{pmatrix}$$

$$=\begin{pmatrix} 3\cos(\psi) +2\sin(\psi) \\ -3\sin(\psi) + 2\cos(\psi) \end{pmatrix} \approx \begin{pmatrix} 3.6056 \\ 0 \end{pmatrix}$$

즉, pitch를 보정하기 위한 값은 3.6056, roll을 보정하기 위한 값은 0이 된다. 따라서 드론은 앞으로 나아가게 된다.
